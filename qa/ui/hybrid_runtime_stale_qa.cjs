/* Hybrid H1/H2 STALE decision-block acceptance. */
...
(async () => {
  ...
  page.on('request', request => {
    ...
    if (decisionPaths.has(pathname)) decisionRequests.push(pathname);
  });
  await page.route('**/api/runtime/health', route => route.fulfill({
    ...
    body: JSON.stringify({
      ...
      last_collection_at: new Date(Date.now() - 3600000).toISOString(),
      data_as_of: new Date(Date.now() - 3600000).toISOString(),
      data_status: 'STALE',
  ...
  try {
    ...
    await page.waitForSelector('#runtimeStatus[data-runtime-state="STALE"]', { timeout: 10000 });
    await new Promise(resolve => setTimeout(resolve, 500));
    const snapshot = await page.evaluate(() => window.Runtime.snapshot());
    const identityReady = snapshot.status === 'STALE' && snapshot.handshakeReady === true;
    console.log((identityReady ? 'PASS' : 'FAIL') +
      ' STALE health keeps identity handshake | ' + JSON.stringify(snapshot));
    const decisionsBlocked = decisionRequests.length === 0;
    console.log((decisionsBlocked ? 'PASS' : 'FAIL') +
      ' STALE blocks current decision requests | ' + JSON.stringify(decisionRequests));
    process.exitCode = identityReady && decisionsBlocked ? 0 : 1;
  } finally {
    await context.close();
    await browser.close();
  }
