...
状态：IMPLEMENTED / INDEPENDENT_CODE_REVIEW_PASSED。基线 `cdf8c1b4114252ead3bf5eb19d2f624be8929bd8`。延续数周至数月波段、持有期间做T、兼有长持/短线，以及持仓/机会并重的已确认画像。
...
第二次复审核验此前RSI/回撤修复和630+8合法向量，仍发现profit_factor/净期望的非零商下溢为0。新增红测后统一增加有限商/乘积检查，并对Brier平方、均值及校准均值/加权误差应用相同已知失败模式防御；真实零与无亏损math.inf的已声明约定保留，普通算术操作顺序不变。聚焦36测试与兼容向量通过，最终精确候选仍需独立复审。这里只是可检测溢出/下溢的防御，不宣称一般浮点计算具有任意精度或全域数学等价。

## 最终状态

以上过程状态为历史快照。最终实现/完整测试/独立复审见 `NUMERICAL-FOUNDATION-VALIDATION-20260914.md`；本轮仅HTTPS/数值基础。后续诊断UI应先核对并行候选，避免再生成第三套同类实现。
