...
> **数值基础进展（2026-09-14）：** N1–N3 HTTPS/指标输入/评价成本与回撤边界及冻结兼容向量已实现并独立审查；不改变既有波段/长持/短线画像或默认评分。详见 `NUMERICAL-FOUNDATION-VALIDATION-20260914.md`。诊断UI及多周期策略有效性必须按后续候选另行核验，真实数据ReadPort和可信绩效仍待完成。
...
> **用户画像已确认（2026-09-11）：** 主线是几周到几个月的波段，持有期间做 T，兼有部分长持和短线；持仓管理与新机会发现为同等重要的双主线。设计详见 `PRODUCT-TRADING-PROFILE-20260911.md`。持仓目的、日内 T 操作及策略评价周期必须分离；此为需求/设计更新，不代表新功能已实现，也不改变风险比例或自动交易限制。

> 全项目再评估继续参见 `PROJECT-REASSESSMENT-20260911.md` 与 `METRIC-AND-ALGORITHM-GOVERNANCE.md`。交易风格和双主线已确认；具体参数、功能实现和全仓语义 Review 仍未完成。
...
> **副标题：A 股优先的个人交易决策驾驶舱**
>
> 文档版本：v1.2-draft（多周期波段与做 T 需求已确认；实现待完成）
>
> 文档状态：User Profile Confirmed / Design Update（保留既有安全合同及历史验收；新多周期功能不冒充已实现）
>
> 对齐日期：2026-09-11（交易画像与双主线更新）
>
...
产品围绕“管理已有持仓”和“发现新机会”两条同等重要的主线解决以下问题；编号不表示永久压低新机会入口：
...
- 资源投入最高；
- 主线交易周期为几周到几个月的波段，不再以 1—20 个交易日作为默认上限；
- 波段期间支持独立的做 T 操作计划，另支持部分长持和短线，分别设置依据、失效条件与评价周期；
- 重点关注板块主线、龙头/中军、趋势回踩、突破延续、二次启动、政策与公告事件；
- 盘中可能做 T 或处理短线，但不是全市场高频/自动交易系统；盘中操作不能擅自改写长周期持仓逻辑。
...
- 盘中会查看系统并可能执行；
- 波段是主线，期间会做 T，另有部分长持和短线；关键机会与风险变化希望尽快获知；
...
- 愿意手动提供：
  ...
  - 持仓股数；
  - 成本价；
- 一般持仓管理不要求补齐完整历史成交记录；启用具体做 T 数量建议需有可信可卖量与时间，进行做 T 归因需记录本次操作的成交腿和成本；
...
波段主仓、长持核心仓、独立短线是持仓目的；做 T 是依附既有持仓的附加操作层，不是另一份独立资金或可卖额度。同一证券多分组共享真实总仓、现金和可卖量。计划持有周期、信号观察周期、盘中触发窗口与交易制度分别建模。详细字段、库存约束、收益口径和反例见 `PRODUCT-TRADING-PROFILE-20260911.md`。
...
当前需求更新：桌面在公共状态条下并列“持仓管理与做 T”和“新机会发现”；移动端首屏提供两个摘要与直达入口。紧急风险可置顶，但不能将新机会长期挤到持仓长列表之后。下方结构树是信息目录，不规定永久的纵向优先级。
...
## 9.8 做 T 与原持仓计划分离（需求已确认，实现待完成）

每个 T 计划引用父持仓/计划，只动用明确分配的机动数量；同时核验账户可卖量、现金、预留、费用、日内峰值敞口及计划失效。普通 A 股做 T 不等于当日新买股票可卖。未完成另一腿、卖飞、额外隔夜仓位必须显示，不能强制买回或回写原波段逻辑。评价包含费用后的配对结果及相对同条件持有的净值增量，不能只看摊薄成本或已实现价差；完整规则来源和验收见 `PRODUCT-TRADING-PROFILE-20260911.md`。
...
用户以几周到几个月波段为主，期间做 T 并保留部分短线；盘中资源优先服务持仓、明确 T 计划和临近触发候选。时效目标仍按实际用途区分：
...
| D02 | 首页核心问题是“今天该怎么操作”；持仓管理与新机会发现同等重要 |
| D03 | 首页采用今日作战简报 + AI 参谋摘要混合 |
| D04 | A 股第一，港股通第二，美股第三 |
| D05 | 模型准确率与交易决策质量同为核心；主周期为几周到几个月波段，做 T、长持和短线分层 |
