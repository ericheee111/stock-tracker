...
> **当前交付（2026-09-15）：** N4a只读评分缺失对照与详情解释、N4b声明周线/实验清单及CLI已完成；实现`7b8816bf6b7f3dcf9272b2039c84318554a855d1`通过独立只读Review，Runtime 998通过/1skip、Quant 753通过、解释Chromium63/63。旧live评分、风险/仓位参数不变，不是研究级数据或有效策略晋级。见`N4-VALIDATION-20260914.md`与`N4-NEXT-STAGES-20260914.md`。下一步先B1d实际字节ReadPort及旧草稿审计；未review的Path/Fill草稿不自动合并。最终推送身份见外置DELIVERY.json，下方历史状态不覆盖本条。
...
- Stage 2H–2J 最终门禁：checkout focused `77 + 52 subtests`、Runtime `522/1`、Quant `628 + 297 subtests`、source-distribution/no-bytecode `3 + 75 subtests`；精确 Index tree `751925dbe0cc7e4b6ad9ab0c1d720f59790ef12a` 通过 focused `77 + 52`、Runtime `522/1`、Quant `626/2 + 222`、H0 `12/12`、H1/H2 `28/28 + 11/11`、H4 `18/18`、Monitor `49/49`、Mock/real Today `17/17`、Portfolio `13/13`、Ruff、compileall、cached-diff 与 25 文件 0 finding scan；实现提交 `ae9036286ee4f40a315891d44e86ab13e4347c41` 已推送；
...
- Stage 4G-R0 实现与独立 Review 提交为 `e5aed357e6127345f3e3de0bab993dc5f895906c`（`fix: close Stage 4G R0 review gaps`），精确 tree 为 `20fad49d582dbc5401b2cc05aec01a67b22a4339`；该 tree 已直接通过 targeted Ruff、focused `114 + 31 subtests`、完整 Quant `723 passed / 2 expected skips + 244 subtests`、完整 Runtime `521 passed / 1 expected skip + 350 subtests` 和 compileall；
